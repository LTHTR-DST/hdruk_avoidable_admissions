...
class FHIRTermClient:
    ...
    def expand_valueset(self, vs_url):
        # expand a valueset
        jsonResponse = None
        response = self.session.get(self.fhir_url + '/ValueSet/$expand', params={"url": vs_url,"property": 'category'})
        if response.status_code != 200:
            print(response.text)
            print(response.status_code)
        else:
    ...
    def expand_valueset_to_list(self, vs_url):
        response = self.expand_valueset(vs_url)
        df_json_concepts = json_normalize(response['expansion']['contains'])
        print(df_json_concepts)
        print(df_json_concepts['code'].tolist())
        return df_json_concepts['code'].tolist()

    def validate_code(self, vs_url, code, system):
        # validate
        jsonResponse = None
        response = self.session.get(self.fhir_url + '/ValueSet/$validate-code', params={"url": vs_url,"code": code, "system": system})
        if response.status_code != 200:
            print(response.text)
            print(response.status_code)
        else:
